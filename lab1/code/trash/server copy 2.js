    ...
    .toArray(function (err, result) {
      ...
      for (let order of result) {
        ...
        for (let product in order.products) {
          formattedOrder["/product/" + product.id] = order.products[product];
        }
...
app.post("/orders", function (req, res, next) {
  ...
  try {
    for (let product of req.body.products) {
      ...
      idArr.push(new ObjectId(product.id));
    }
    console.log(idArr);
  } catch (err) {
    console.log(err);
  ...
  db.collection("products").updateMany(
    ...
    function (err, result) {
      ...
      if (result.matchedCount < idArr) {
        res.status(401).send("Error adding to database, product not in stock");
        return;
      }

      db.collection("orders").insert(
        { name: req.body.name, products: req.body.products },
        function (err, result) {
          if (err) {
