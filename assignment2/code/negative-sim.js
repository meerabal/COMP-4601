...
// item based, considering negative similarities
// default set to top-K, neighbourhood size = 5
...
const pred = (user, prod) => {
  ...
  for (let n = 0; n < neighbourhoodSize; n++) {
    ...
    for (let index = 0; index < simKeys.length; index++) {
      ...
      if (maxVal <= Math.abs(sim[prod][i])) {
        maxVal = Math.abs(sim[prod][i]);
        maxI = i;
      }
    }
  ...
  if (count === 0) {
    ...
    for (let index = 0; index < count; index++) {
      ...
      let rui = r[user][i];
      num += Math.abs(sim[prod][i] * rui);
      denom += Math.abs(sim[prod][i]);
    }
  }
